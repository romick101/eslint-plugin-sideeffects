...
function containsArgument(node, args) {
    return node.type === 'MemberExpression' ?
        node.object.type === 'MemberExpression' ? containsArgument(node.object, args) :
        args.find(el =>
            el.name === (node.object ? node.object.name : node.name)) :
        args.find(el => el.name === node.name);
}

function isMutatingArgument(node, args) {
    return containsArgument(node, args) && isMutatingMethod(node.property);
}
    ...
    create: function (context) {
        ...
        return {
            ...
            [objectAssignMatcher]: node => {
                if (containsArgument(node.parent.arguments[0], args))
                    context.report({
                        node,
